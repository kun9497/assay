...
않는다고 했지만, 틀린 게시판을 잰 결과였습니다. KNVD 자체 공개는 국내 상용 소프트웨어 173건이지만
**보안공지는 2,422건**이고 Apache, OpenSSL 같은 소프트웨어의 CVE를 키로 합니다. 표본 3건은 셋 다
assay가 이미 가지고 있는 권고이며, 실제 컨테이너에 들어가는 패키지입니다. KISA를 독립 매칭 소스로
쓸 수는 없지만, assay가 OSV로 매칭한 CVE에 KISA의 한국어 제목과 조치 정보를 붙이는 것은
동작합니다. 그쪽이 방향입니다(D27의 메커니즘에 출처 하나 추가).
...
`db update`는 설정된 평가 출처 — 오늘은 NVD (D27) — 도 권고 provider 바로 뒤에 실행하여 그
...
선택 사항이며 필수가 아닙니다. 키가 없어도 `db update`는 NVD를 동기화하되, 더 느릴 뿐입니다.
...
**48%를 high 이상**으로 평가했습니다. assay 자기 바이너리 스캔이 그 대가를 보여줍니다 — finding
3건이 전부 무평가라 `--fail-on low`조차 0을 반환하는데, NVD는 그중 둘을 7.8과 5.3으로 매깁니다.
조인 키는 CPE가 아니라 CVE입니다. NVD는 매치 데이터를 `vendor:product`로 키잡는데 어떤 purl로도
유도되지 않고, 학습된 사전은 Alpine 85%인 반면 Go는 11%입니다 — 8,029건 중 4,125건이 Go인데도요.
무평가 권고 1,008건은 CVE가 아예 없어 어떤 설계로도 unknown으로 남습니다 (D27).

- [ ] NVD provider — 2.0 API 벌크 동기화, 187 요청, API 키 불필요
- [ ] CVE로 조인한 rating, 판정은 최고 밴드 (D25의 메커니즘)- [ ] 두 번째 평가 출처로서의 NVD (`docs/deferred-decisions.md` 참조)
...
실재하는 기능이고 동시에 소박한 기능입니다. 미해결이 둘 남아 있습니다. 라이선스(사이트가 공공누리
표시 없이 모든 권리 보유이며, 이는 만든 데이터베이스를 재배포할 때 문제이지 그것으로 스캔하는
데에는 아닙니다), 그리고 의존성 없이 HTML에서 영향/해결 표를 파싱하는 문제입니다. 자세한 내용은
로드맵에 있습니다.

- [ ] KNVD provider와 CVE 기반 보강 조인
