...
`NVD_SINCE_DAYS`는 **한 번의 빌드** 범위를 제한할 뿐 매일의 델타가 아닙니다. `db update`가 빈
상태에서 다시 빌드하므로, 범위를 제한한 실행의 창이 그 데이터베이스의 전체 NVD 커버리지입니다.
`db status`는 그것을 `COVERED` 열로 출력해서, 일부만 담긴 데이터베이스가 완전한 것처럼 보이지
않게 합니다. 진짜 델타는 다음 슬라이스의 몫입니다.

**⑧ 배포되는 데이터베이스** — 빌더가 느린 동기화를 돌고, 나머지는 결과를 받습니다. **다음 작업.**
재검토 조건이 "CI 재빌드 시간이 병목이 될 때"였던 보류 결정이고, 위 측정이 그것을 발동시켰습니다.
grype와 trivy 모두 이렇게 동작합니다.

- [ ] 데이터베이스를 중앙에서 빌드하고 OCI 아티팩트로 배포
- [ ] `assay db update`가 재빌드 대신 배포된 아티팩트를 받아옴
- [ ] 전체 한 번 위에 매일 델타를 쌓기 (지금은 빈 상태에서 재빌드)
