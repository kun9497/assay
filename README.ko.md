...
데이터베이스 쪽에서 네트워크를 건드리는 명령은 `assay db update`와 `assay db build` 둘뿐입니다 —
**스캔 자체는 여전히 네트워크를 전혀 쓰지 않습니다**, 사용자가 지목한 원격 **대상**을 받아오는
것을 빼면요(D14). `assay db build`는 업스트림 provider들로부터 직접 빌드합니다. 이제 배포자의
명령이지 평상시 경로가 아닙니다 — 누가 이것이 필요하고 왜인지는 아래
[데이터베이스](#데이터베이스) 절을 보세요.
...
않습니다**(D14). 데이터베이스 쪽에서 네트워크를 건드리는 명령은 `db update`와 `db build`
둘뿐입니다.
...
```bash
NVD_ENABLE=1 assay db build          # 전체 ~7시간, 로컬에서, 잡 상한 없이
