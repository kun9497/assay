...
시작할 때의 차별점은 데이터와 부딪혀 살아남지 못했습니다. KISA/KNVD를 일급 provider로 삼을
계획이었고, NVD와 OSV가 늦게 다루거나 아예 다루지 않는 소프트웨어를 커버한다는 전제였습니다.
2026-08-02 조사 결과, KNVD가 발행한 레코드는 전체 173건이고 전부 국내 상용 소프트웨어이며 Go, npm,
PyPI, Alpine 패키지는 하나도 없습니다. 컨테이너나 소스 트리에 대한 CVE 조인은 사실상 걸리지
않습니다. 슬라이스 5와 `docs/deferred-decisions.md`에 기록했고, 그 전제를 여기 조용히 남겨두지
않습니다.
