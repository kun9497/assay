...
**맨경로는 내용으로 분류합니다.** 순서는 고정입니다 — `debug/buildinfo`, 그다음 CycloneDX, 그다음
"디렉터리인가". 각 판별은 싸고 명확하므로(buildinfo는 헤더만 읽고 Go 바이너리가 아니면 즉시 실패)
순서는 비용이 아니라 결정성의 문제입니다. 어디에도 맞지 않는 경로는 셋을 모두 이름 붙인 오류이지,
그중 하나로 조용히 흘러가지 않습니다.
