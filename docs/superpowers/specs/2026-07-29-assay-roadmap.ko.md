...
**살펴본 레코드 어디에도 KVE 식별자가 없었습니다.** 공개된 항목은 모두 CVE를 제목과 키로 씁니다 —
`CVE-2026-24498 | EFM-Networks ipTIME …` — 상세 페이지도 CVE만 인용합니다. 173건 중 10건이므로
전수 조사가 아니라 표본이지만, §1의 전제("KISA/KNVD가 권고와 KVE 식별자를 발행한다")와 D3의
"CVE↔KVE 조인"이 KVE 쪽에 조인할 대상을 갖지 못할 수 있다는 것을 표시하기에는 충분합니다. 여기에
의존하는 것을 만들기 전에 확인이 필요합니다.
