...
*[English](DESIGN.md) · 한국어* · [← README](../README.ko.md)
...
`pnpm-lock.yaml`과 yarn berry 락파일은 D103부터 읽습니다(D61 때는 인식하고 이름을 부르며
거부했습니다. 당시 읽으려면 YAML 의존성이 들었을 텐데, D102의 yaml.v3 승격이 그 비용을
없앴습니다). pnpm은 `packages:` 섹션에서만 읽으며, lockfileVersion 5.4/6.0/9.0과 다중 문서
파일 전체를 아우릅니다. berry의 이름은 각 항목의 키가 아니라 `resolution:`에서 가져오는데,
그래야 별칭 붙은 항목이 실제 디스크에 있는 패키지와 맞아떨어집니다. workspace/link 항목은
보여지되 게이트에는 걸리지 않고, git과 malformed 항목은 진짜 불완전성이므로 게이트에
걸립니다. v1 문법도 berry도 아닌 `yarn.lock`은 알 수 없는 방언으로 거부합니다 — v1 파서는
낯선 입력에서 실패하지 않고 성공한 뒤 아무것도 못 찾는데, 이건 이 프로젝트가 어디서나
거부하는 바로 그 혼동입니다.
...
seed를 사용하는 일일 NVD 갱신은 마지막 성공 요청의 끝 시점에서 하루를 중첩해 이어받으며,
필요하면 설정된 기간을 늘립니다. 복구 구간이 120일을 넘으면 연결된 API 요청 구간으로 나눕니다.
명시적인 `NVD_UNTIL_DAYS` 백필은 요청한 범위를 유지합니다. 이전 seed는 NVD의 `DataAsOf`를
체크포인트로 사용하며, 시점이 없는 seed는 알 수 없는 기간을 건너뛰는 대신 거부합니다.

`--ratings-only`는 최신 전체 목록을 가져오기 전에 seed의 EPSS·KEV 행을 제거합니다.
해당 annotator를 비활성화한 경우에도 제거하며, NVD rating은 증분으로 유지합니다.

`db push`는 실제 advisory 건수, provider·생태계 목록, 출처별 rating 건수를 비교합니다.
기존 advisory provider·릴리스 누락, 빈 DB로 교체, 전체 또는 생태계별 advisory의 20% 초과 감소는
`--force`가 필요합니다. 작은 삭제와 EPSS·KEV 최신 목록의 감소는 허용하지만 rating 출처 누락이나
NVD rating 감소는 거부합니다. 건수 정보가 없는 이전 artifact는 내려받아 비교합니다.
레지스트리 조회·메타데이터 오류도 `--force`로 명시적으로 재정의하지 않으면 배포를 중단합니다.
이 건수 검사는 큰 누락을 감지하며, 모든 의미상의 회귀를 보장하지는 않습니다.

`EPSS_ENABLE=0`이나 `KEV_ENABLE=0`으로 실행한 빌드는 그 출처의 키를 아예 갖지 않습니다 —
해당 행을 쓰는 것은 오직 그 annotator뿐이고, seed 복사본도 `--ratings-only`도 그 행을
보존하지 않기 때문입니다 — 그래서 `db push`는 그 값을 갖고 있던 기준선과 비교해 그 아티팩트를
거부합니다. 이는 `--ratings-only`에 국한되지 않으며, 어떤 빌드든 마찬가지입니다. annotator가
돌았지만 아무것도 평가하지 못한 경우는 다른 상태이고 허용됩니다. 건수가 0인 채로 항목은
존재하기 때문입니다. 두 출처 모두 기본값이 켜짐이므로 nightly는 영향을 받지 않습니다.
`--force`가 명시적인 탈출구이며, 빌드는 이 문제로 더 일찍 실패할 수 없습니다. push 대상
자체를 받지 않으므로 자신이 무엇과 비교될지 알 도리가 없기 때문입니다.

Ubuntu tracker clone·fetch는 일시적 네트워크 오류를 최대 네 번 시도하며, 대기 간격은 1·3·10초,
시도별 제한 시간은 10분입니다. 실패한 clone은 전용 임시 디렉터리에서 정리합니다.
두 DB 워크플로는 `queue: max`를 사용해 새 대기 작업이 이전 대기 작업을 대체하지 않게 합니다.
큐가 가득 찬 경우나 job 제한 시간을 넘긴 경우에는 여전히 실패할 수 있습니다.
...
[`docs/deferred-decisions.ko.md`](deferred-decisions.ko.md)에 있습니다.
...
- [x] pnpm과 yarn berry를 읽습니다 (D103) — pnpm 5.4/6.0/9.0을 `packages:`에서만, berry
      이름은 `resolution:`에서; local 항목은 보여지되 target 게이트 대상은 아니고, git/malformed는
      게이트 대상입니다; 규칙은 실제 항목 약 11,000개에 대고 기계 검증했고 뮤테이션 8/8이 red입니다
...
[`docs/deferred-decisions.ko.md`](deferred-decisions.ko.md)를 확인해 주십시오 — 눈에 띄는
